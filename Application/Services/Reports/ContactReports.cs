...
namespace Application.Services.Reports
{
    ...
    {
        public async Task<GenericResponse<Dictionary<string, object>>> GenerateContactReportAsync(GenerateReportDto reportDto)
        {
            try
            {
                ...
                var contacts = await _reportRepository.ExecuteStoredProcedureAsync("sp_Contacts_Report", parameters);
                ...
                {
                    { "ReportData", contacts },         // Data populated with JSON string
