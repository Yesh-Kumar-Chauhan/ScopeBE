...
namespace Application.Services.Reports
{
    ...
    {
        public async Task<GenericResponse<Dictionary<string, object>>> GenerateEmergencyClosingAsync(GenerateReportDto reportDto)
        {
            try
            {
                ...
                var emergencyClosing = await _reportRepository.ExecuteStoredProcedureAsync("sp_Closings_Report", parameters);

                // Check if the data is empty
                if (emergencyClosing == null || !emergencyClosing.Any())
                {
                ...
                {
                    { "ReportData", emergencyClosing },         // Data populated with JSON string
